class Tree:
    ...
    def __init__(self, commands):
        ...
        while index < len(commands):
            ...
            index += 1
        self.space_to_free = 30_000_000 - (70_000_000 - self.root.size)
        self.emprime()
    ...
    def smallest_large_dir(self):
        return self.__smallest_large_dir(self.root, self.root).size

    def __smallest_large_dir(self, curr, smallest):
        
        if isinstance(curr, Tree.Dir):
            if curr.size >= self.space_to_free and curr.size < smallest.size:
                smallest = curr

        for filho in curr.filhos:
            smallest = self.__smallest_large_dir(filho, smallest)
...
print(files.smallest_large_dir())
