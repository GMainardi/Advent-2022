...
class Monkey:
    ...
    def update_magic_number(self, magic_number):
        self.magic_number = magic_number
    def analysis_item(self, item):
        self.insp_count += 1
        # print(f'\tMonkey inspects an item with a worry level of {item}.')
        worry_level = WorryLevelCalculator(self.calc, item).eval() % magic_number
...
for monkey in monkeys:
    magic_number *= monkey.test
print(magic_number)

for monkey in monkeys:
    monkey.update_magic_number(magic_number)
